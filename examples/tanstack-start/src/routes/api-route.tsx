  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const leak = url.searchParams.get('leak') === 'true'
        const leaked = leak ? ENV.SENSITIVE_VAR : undefined
...
function ApiRoutePage() {
  ...
  const handleFetch = async (leak: boolean) => {
    setLoading(true)
    try {
      const res = await fetch(`/api-route${leak ? '?leak=true' : ''}`)
  ...
  return (
    <main className="page-wrap px-4 pb-8 pt-14">
      <h1 className="display-title mb-6 text-3xl font-bold text-[var(--sea-ink)]">
        API Route Handler Env Vars
      </h1>
      <p className="mb-6 text-[var(--sea-ink-soft)]">
        Fetches all env vars via a GET route handler. Values are redacted —
        only showing whether they are set and their length.
      </p>
